...
T = 0.1; alpha = 1 * 2/(T*eigL(end));                           % sampling_time, gain of control input
...
function vonglap = find_index(x_record)
    ...
    for k = 1:tong_so_vonglap
        ...
        % Chuyển thành ma trận [tac_tu x trang_thai] dạng 10x2
        x_matrix = reshape(snapshot, [so_trang_thai, so_tac_tu])';
        trungbinh = mean(x_matrix, 1);         % [mean_1, mean_2]
        max_lech = max(abs(x_matrix - trungbinh), [], 1);  % lệch lớn nhất theo cột
