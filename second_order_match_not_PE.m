...
exportgraphics(gcf, 'second_order_matched_state4.pdf', ...
...
exportgraphics(gcf, 'second_order_matched_velocity4.pdf', ...
    'BackgroundColor', 'white', ...
    'ContentType', 'vector'); 
...
axis normal;
grid on;
exportgraphics(gcf, 'second_order_matched_first_disturbance4.pdf', ...
    'BackgroundColor', 'white', ...
    'ContentType', 'vector');
...
axis normal;
grid on;
exportgraphics(gcf, 'second_order_matched_second_disturbance4.pdf', ...
    'BackgroundColor', 'white', ...
    'ContentType', 'vector');
...
exportgraphics(gcf, 'second_order_matched_trajectories4.pdf', ...
    'BackgroundColor', 'white', ...
    'ContentType', 'vector');
Q_dif_record = [];
...
for i = 0:500
    ...
    d_hat_k = d_hat_record(20*i+1 : 20*i+20, :);                
    diff_k = Q_k * (d_hat_k - d);                  
    Q_dif_record = [Q_dif_record; diff_k'];
end
...
for i = 1:size(Q_dif_record, 2)
    plot(0:500, Q_dif_record(:, i),'color','b', 'DisplayName', sprintf('$[Q(\hat{d}-d)]_{%d}$', i));
end

xlabel('$k\ [\mathrm{iteration}]$', 'Interpreter', 'latex', 'FontSize', 18);
ylabel('$Q[k](\hat{d}[k] - d)$', 'Interpreter', 'latex', 'FontSize', 18);

title('Evolution of $Q[k](\hat{d}[k] - d)$', 'Interpreter', 'latex');
set(gca, 'FontSize', 16);
grid on;
xlim([0 500]);
...
ylim([-1 1]);
xlim([0 100]);
exportgraphics(gcf, 'second_order_matched_not_PE.pdf', ...
    'BackgroundColor', 'white', ...
    'ContentType', 'vector');
